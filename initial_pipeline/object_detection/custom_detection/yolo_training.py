...
def train_fold(fold_num, yaml_path, device, runs_dir, timestamp):
...
def main():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")  # Shorter timestamp format
    runs_dir = script_dir / "cv_runs" / timestamp  # Shorter path
    runs_dir.mkdir(parents=True, exist_ok=True)
    
    # Create folds directory
    folds_root = runs_dir / "folds"  # Shorter name
    ...
    all_images, dataset_root, data_config = collect_all_data(script_dir / DATA_YAML)
    ...
    for fold_num, yaml_path, test_images in fold_configs:
        metrics, best_path = train_fold(fold_num, yaml_path, device, runs_dir, timestamp)
